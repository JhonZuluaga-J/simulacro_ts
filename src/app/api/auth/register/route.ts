...
function validateBody(body: unknown): RegisterBody {
...
function unknownErrorResponse(error: unknown): NextResponse {
  ...
  logger.error("Unhandled registration error", { error: message });
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const { user, accessToken, refreshToken } = await register(validated);
